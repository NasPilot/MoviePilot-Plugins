...
def get_download_url(pickcode: str, cookie: str):
    ...
    Raises:
        HTTPError: 请求失败
        OSError: 接口返回错误
        ValueError: 参数错误或数据解析错误
    """
    if not pickcode or not cookie:
        raise ValueError("pickcode和cookie不能为空")
...
def save_mediainfo_file(
    ...
    try:
        ...
        # 下载文件
        logger.info(f"【媒体信息文件】开始下载: {file_name}")
        with requests.get(
            download_url,
            stream=True,
            timeout=30,
            ...
            with open(file_path, "wb") as f:
                for chunk in response.iter_content(chunk_size=8192):
                    ...
                    if current_time - last_log_time > 3 and total_size > 0:
                        progress = (downloaded / total_size) * 100
                        logger.debug(f"【媒体信息文件】{file_name} 下载进度: {progress:.2f}%")
                        last_log_time = current_time
        
        logger.info(f"【媒体信息文件】保存 {file_name} 文件成功: {file_path}")
        return True
    except HTTPError as e:
        logger.error(f"【媒体信息文件】下载失败 {file_name}: HTTP错误 - {str(e)}")
        return False
    except OSError as e:
        logger.error(f"【媒体信息文件】保存失败 {file_name}: 文件操作错误 - {str(e)}")
        return False
    except Exception as e:
        logger.error(f"【媒体信息文件】处理异常 {file_name}: {str(e)}")
        return False
...
class FullSyncStrmHelper:
    ...
    def __init__(
        ...
        Args:
            client: 115网盘客户端
            user_rmt_mediaext: 媒体文件扩展名，逗号分隔
            user_download_mediaext: 需要下载的媒体信息文件扩展名，逗号分隔
            server_address: 服务器地址
            cookie: 用户cookie
            auto_download_mediainfo: 是否自动下载媒体信息文件
        """
        self.rmt_mediaext = [
            f".{ext.strip()}" for ext in user_rmt_mediaext.replace("，", ",").split(",") if ext.strip()
        ]
        self.download_mediaext = [
            f".{ext.strip()}"
            for ext in user_download_mediaext.replace("，", ",").split(",") if ext.strip()
        ]
    ...
    def generate_strm_files(self, full_sync_strm_paths: str) -> bool:
        """
        生成 STRM 文件
        
        Args:
            full_sync_strm_paths: 配置的路径映射，格式为"本地路径#网盘路径"，多行用换行符分隔
            
        Returns:
            bool: 是否全部成功
        """
        if not full_sync_strm_paths or not full_sync_strm_paths.strip():
            logger.error("【全量STRM生成】未配置有效的路径映射")
            return False
            
        media_paths = [p for p in full_sync_strm_paths.split("\n") if p.strip()]
        if not media_paths:
            logger.error("【全量STRM生成】未配置有效的路径映射")
            return False
            
        logger.info(f"【全量STRM生成】开始处理 {len(media_paths)} 个路径映射")
        success_count = 0
        
        for path_index, path in enumerate(media_paths):
            logger.info(f"【全量STRM生成】处理第 {path_index+1}/{len(media_paths)} 个路径映射: {path}")
            
            try:
                parts = path.split("#", 1)
                if len(parts) != 2:
                    logger.error(f"【全量STRM生成】路径格式错误: {path}，应为'本地路径#网盘路径'格式")
                    continue
                    
                target_dir = parts[0].strip()
                pan_media_dir = parts[1].strip()
                
                if not target_dir or not pan_media_dir:
                    logger.error(f"【全量STRM生成】路径配置不完整: {path}")
                    continue
                ...
                try:
                    ...
                    logger.error(f"【全量STRM生成】网盘媒体目录不存在: {pan_media_dir}, 错误: {e}")
                    continue
                except Exception as e:
                ...
                try:
                    ...
                    logger.error(f"【全量STRM生成】创建本地目录失败: {target_dir}, 错误: {e}")
                    continue
                    
                # 获取文件列表并计算总数
                try:
                    files_list = list(iter_files_with_path(self.client, cid=parent_id, cooldown=2))
                    self.total_files += len(files_list)
                    logger.info(f"【全量STRM生成】网盘目录 {pan_media_dir} 共有 {len(files_list)} 个文件待处理")
                except Exception as e:
                    logger.error(f"【全量STRM生成】获取网盘文件列表失败: {e}")
                    continue
                    
                # 处理文件
                path_success = self._process_files(files_list, target_dir, pan_media_dir)
                if path_success:
                    success_count += 1
                    
            except Exception as e:
                logger.error(f"【全量STRM生成】处理路径映射异常: {path}, 错误: {str(e)}")
                
        # 输出最终统计信息
        logger.info(
            f"【全量STRM生成】全量生成完成，共处理 {self.processed_files} 个文件，"
            f"生成 {self.strm_count} 个STRM文件，下载 {self.mediainfo_count} 个媒体数据文件，"
            f"跳过 {self.skipped_count} 个文件，失败 {self.error_count} 个文件"
        )
        
        return success_count == len(media_paths)
        
    def _process_files(self, files_list: list, target_dir: str, pan_media_dir: str) -> bool:
        """
        处理文件列表
        ...
        try:
            for item in files_list:
                ...
                # 显示进度
                current_time = time.time()
                if current_time - self.last_progress_time > 5 and self.total_files > 0:
                    progress = (self.processed_files / self.total_files) * 100
                    logger.info(f"【全量STRM生成】总进度: {progress:.2f}% ({self.processed_files}/{self.total_files})")
                ...
                try:
                    ...
                    # 处理媒体信息文件下载
                    if self.auto_download_mediainfo and file_path.suffix in self.download_mediaext:
                        self._download_mediainfo_file(item, file_path, original_file_name)
                        continue
                    ...
                    # 生成STRM文件
                    self._generate_strm_file(item, file_path, file_target_dir, original_file_name)
                    
                except Exception as e:
                    logger.error(f"【全量STRM生成】处理文件异常: {item.get('path', '未知')}, 错误: {str(e)}")
                    self.error_count += 1
                    
            return True
        except Exception as e:
            logger.error(f"【全量STRM生成】处理文件列表异常: {str(e)}")
            return False
            
    def _download_mediainfo_file(self, item: dict, file_path: Path, original_file_name: str) -> None:
        """
        下载媒体信息文件
        """
        try:
            ...
            if not pickcode:
                logger.error(f"【全量STRM生成】{original_file_name} 不存在 pickcode 值，无法下载该文件")
                self.error_count += 1
                return
                
            download_url = get_download_url(pickcode=pickcode, cookie=self.cookie)
            if not download_url:
                logger.error(f"【全量STRM生成】{original_file_name} 下载链接获取失败，无法下载该文件")
                self.error_count += 1
                return

            success = save_mediainfo_file(
                file_path=file_path,
            ...
            logger.error(f"【全量STRM生成】下载媒体信息文件异常: {original_file_name}, 错误: {str(e)}")
            self.error_count += 1
            
    def _generate_strm_file(self, item: dict, file_path: Path, file_target_dir: Path, original_file_name: str) -> None:
        """
        生成STRM文件
        """
        try:
            ...
            pickcode = item.get("pickcode") or item.get("pick_code")
            if not pickcode:
                logger.error(f"【全量STRM生成】{original_file_name} 不存在 pickcode 值，无法生成 STRM 文件")
                self.error_count += 1
                return
                
            # 验证pickcode格式
            if not (len(pickcode) == 17 and str(pickcode).isalnum()):
                logger.error(f"【全量STRM生成】错误的 pickcode 值 {pickcode}，无法生成 STRM 文件")
                self.error_count += 1
                return
            ...
            logger.error(f"【全量STRM生成】生成 STRM 文件异常: {original_file_name}, 错误: {str(e)}")
            self.error_count += 1
    ...
    def get_generate_total(self) -> Tuple[int, int, int, int]:
        ...
        Returns:
            Tuple[int, int, int, int]: STRM文件数, 媒体信息文件数, 跳过文件数, 错误文件数
        """
        return self.strm_count, self.mediainfo_count, self.skipped_count, self.error_count
