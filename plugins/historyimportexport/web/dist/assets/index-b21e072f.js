...
import { propsFactory, includes, isOn, eventName, genericComponent, getCurrentInstance, omit, makeLayoutProps, makeThemeProps, provideTheme, createLayout, useRtl, deceleratedEasing, standardEasing, acceleratedEasing, provideDefaults, convertToUnit, destructComputed, isCssColor, isParsableColor, parseColor, getForeground, getCurrentInstanceName, isObject, onlyDefinedProps, SUPPORTS_INTERSECTION, clamp, consoleWarn, makeLayoutItemProps, useProxiedModel, useToggleScope, useLayoutItem, deepEqual, wrapInArray, findChildrenWithProvide, IconValue, useTheme, useIcon, flattenFragments, useResizeObserver, IN_BROWSER, hasEvent, useLocale, EventProp, filterInputAttrs, matchesSelector, callEvent, pick, makeDisplayProps, useDisplay, useGoTo, focusableChildren, consoleError, defineComponent, deprecate, getPropertyFromItem, isPrimitive, focusChild, CircularBuffer, defer, templateRef, isClickInsideElement, getNextElement, debounce, camelizeProps, ensureValidVNode, checkPrintable, noop, pickWithRest, keys, getEventCoordinates, HSVtoRGB, RGBtoHSV, HSVtoHSL, HSLtoHSV, HSVtoHex, HexToHSV, has, getDecimals, createRange, keyValues, SUPPORTS_EYE_DROPPER, HSVtoCSS, RGBtoCSS, getContrast, escapeForRegex, isComposingIgnoreKey, getObjectValueByPath, isEmpty, defineFunctionalComponent, breakpoints, useDate, createDateRange, humanReadableFileSize, provideLocale, useLayout, extractNumber, VuetifyLayoutKey, refElement, renderSlot, VClassIcon, VComponentIcon, VLigatureIcon, VSvgIcon } from './date-9c760b4d.js';
import Page from './__federation_expose_Page-017e6e2c.js';
...
function useRules(fn) {
  const rules = inject$k(RulesSymbol, null);
  if (!fn) {
    if (!rules) {
  ...
  return rules?.resolve(fn) ?? toRef$K(fn);
}
...
const VInput = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    const hasMessages = toRef$J(() => messages.value.length > 0);
    const hasDetails = toRef$J(() => !props.hideDetails || props.hideDetails === 'auto' && (hasMessages.value || !!slots.details));
    const messagesId = computed$1v(() => hasDetails.value ? `${id.value}-messages` : undefined);
    ...
    useRender(() => {
      ...
      return _createElementVNode$1v("div", {
        ...
        "name": "append",
        "color": iconColor.value
      }, null), slots.append?.(slotProps.value)]), hasDetails.value && _createElementVNode$1v("div", {
        "id": messagesId.value,
...
function connectedLocationStrategy(data, props, contentStyles) {
  ...
  watch$y(data.target, (newTarget, oldTarget) => {
  ...
  watch$y(data.contentEl, (newContentEl, oldContentEl) => {
...
const VField = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    const messagesId = toRef$z(() => !props.details ? undefined : `${id.value}-messages`);
...
const useSlider = _ref => {
  ...
  const thumbLabelColor = computed$13(() => props.error || props.disabled ? undefined : props.thumbColor);
  ...
  const data = {
    ...
    max,
    mousePressed,
    noKeyboard: toRef$s(() => props.noKeyboard),
    numTicks,
    onSliderMousedown,
    onSliderTouchstart,
    parsedTicks,
...
const {normalizeClass:_normalizeClass$O,normalizeStyle:_normalizeStyle$E,createElementVNode:_createElementVNode$10,withDirectives:_withDirectives$6,vShow:_vShow$2,createVNode:_createVNode$1b} = await importShared('vue');
    ...
    useRender(() => {
      ...
      return _createElementVNode$10("div", {
        ...
        default: () => [_withDirectives$6(_createElementVNode$10("div", {
          ...
          "class": _normalizeClass$O(['v-slider-thumb__label', backgroundColorClasses.value]),
          "style": _normalizeStyle$E(backgroundColorStyles.value)
        }, [_createElementVNode$10("div", null, [slots['thumb-label']?.({
          modelValue: props.modelValue
        }) ?? props.modelValue.toFixed(step.value ? decimals.value : 1)]), _createElementVNode$10("div", {
...
const {createElementVNode:_createElementVNode$M,createVNode:_createVNode$W,normalizeClass:_normalizeClass$C,normalizeStyle:_normalizeStyle$u} = await importShared('vue');
  ...
  setup(props, _ref) {
    ...
    useRender(() => {
      ...
      return _createVNode$W(props.tag, {
        ...
        default: () => [slots.top?.(), _createVNode$W(VDefaultsProvider, {
          "defaults": tableContentDefaults
        }, {
          default: () => [slots.default ? _createElementVNode$M("div", {
            "class": "v-table__wrapper",
            "style": {
              height: convertToUnit(props.height)
            }
          }, [_createElementVNode$M("table", null, [slots.default()])]) : slots.wrapper?.()]
        }), slots.bottom?.()]
      });
    });
    return {};
