...
# 创建缓存实例
cache_backend = Cache(cache_type='ttl', maxsize=100000, ttl=60 * 60 * 24 * 3)
...
def cache_with_logging(region, source):
    ...
    def decorator(func):
        ...
        def wrapped_func(*args, **kwargs):
            # 生成缓存键
            key = str(hashkey(func.__name__, args, kwargs))
            ...
            if result is None:
                ...
                cache_backend.set(key, "None", ttl=60 * 5, region=region)
            else:
                # 结果不为 None，使用默认 TTL 缓存
                cache_backend.set(key, result, ttl=60 * 60 * 24 * 3, region=region)
